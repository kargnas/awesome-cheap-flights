...
from awesome_cheap_flights import SearchConfig, execute_search
...
def normalize_departures(raw: Any) -> Dict[str, str]:
    ...
    if isinstance(raw, dict):
        return {
            strip_comment(k): strip_comment(v).upper()
            for k, v in raw.items()
            if strip_comment(k) and strip_comment(v)
        }
...
def normalize_itineraries(raw: Any) -> List[Tuple[str, str]]:
    ...
    for item in items:
        ...
        raise ValueError(f"Invalid itinerary entry: {item}")
    return [(o, i) for o, i in pairs if o and i]
